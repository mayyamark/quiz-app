...
const questionCreateSchema = {
  ...
  points: (value) => {
    ...
    if (pointsAsNumber === Number.NaN || pointsAsNumber < 1 || pointsAsNumber > 10) {
  ...
  answers: (value) => {
    ...
    const errors = value.map(answer => schemaValidator(answerCreateSchema, answer)).filter(error => error != null);
